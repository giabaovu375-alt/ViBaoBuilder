...
export const Route = createFileRoute("/export/$projectId")({
  head: () => ({ meta: [{ title: "Export — Slide Builder" }] }),
    ...
    <div className="min-h-screen bg-background">
      <header className="flex items-center justify-between border-b px-4 py-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold">{project.name} — Export</span>
        </div>
...
function elHtml(el: Element): string {
  ...
  switch (el.type) {
    case "text":
      return `<div style="${base}font-size:${el.fontSize}px;color:${el.color};">${esc(el.content)}</div>`;
    case "image":
      return `<img src="${esc(el.src)}" alt="${esc(el.alt)}" style="${base}object-fit:cover;" />`;
    case "button":
      return `<a href="${esc(el.href)}" style="${base}display:inline-flex;align-items:center;justify-content:center;background:${el.bg};color:${el.color};border-radius:6px;text-decoration:none;font-weight:500;">${esc(el.label)}</a>`;
    case "section":
      return `<div style="${base}background:${el.bg};border-radius:8px;"></div>`;
  }
}
